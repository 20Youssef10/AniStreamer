...
import { Play, Pause, Maximize, Minimize, Volume2, VolumeX, SkipForward, Layers, Monitor, Lightbulb, LightbulbOff, Mic, MicOff, FastForward, RotateCcw, RotateCw, Settings, Gauge, PictureInPicture, Settings2, X, Sliders, Activity, ZoomIn, ZoomOut, Check, Camera, Zap } from 'lucide-react';
...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ sources = [], onEnded, onNext, onProgress, poster, title, malId, episodeNumber, onTheaterMode, isTheater }) => {
    ...
    useEffect(() => {
        ...
        if (isHLS && Hls.isSupported()) {
            ...
            hls.on(Hls.Events.MANIFEST_PARSED, (event, data) => {
                ...
                const sorted = Array.from(uniqueLevels.entries())
                                    .map(([height, level]) => ({ height, level }))
    ...
    const handleTimeUpdate = () => {
        ...
        // Auto Sync at 80%
        if (currentProgress > 80 && !hasSyncedRef.current) {
    ...
    const resetFilters = () => {
        setFilters({ brightness: 100, contrast: 100, saturate: 100, hue: 0, blur: 0 });
        setZoom(1);
    };
    ...
    const takeSnapshot = () => {
        ...
        if (ctx) {
            ...
            link.href = dataUrl;
            link.download = `anistream_snap_${Date.now()}.png`;
            link.click();
            showToast("Snapshot Saved!", "success");
        }
    };

    // 9. Audio Boost Function
    const toggleAudioBoost = () => {
        ...
        if (!boostEnabled) {
            gainNodeRef.current!.gain.value = 3; // 300% volume
            setBoostEnabled(true);
            showToast("Audio Boost Active (300%)", "info");
        } else {
            gainNodeRef.current!.gain.value = 1; // 100% volume
            setBoostEnabled(false);
            showToast("Audio Boost Disabled", "info");
        }
    };
    ...
    return (
        ...
        >
            ...
            {activeSkip && (
                <button 
                    onClick={performSkip}
                    className="absolute bottom-24 left-4 z-50 bg-white text-black px-6 py-3 rounded-full font-bold shadow-lg animate-bounce flex items-center gap-2 hover:bg-slate-200 transition-colors"
                >
            ...
            {currentSource.type === 'embed' ? (
                <iframe src={currentSource.url} className="w-full h-full relative z-30" allowFullScreen allow="autoplay" />
            ) : (
                <div className="w-full h-full relative z-30 overflow-hidden bg-black flex items-center justify-center">
                    <video
                        ...
                        muted={muted} 
                        playsInline
                        crossOrigin="anonymous" // Important for Snapshot
                    />
                </div>
            )}
                ...
                <div className="absolute top-0 right-0 h-full w-72 bg-dark-900/95 backdrop-blur-xl border-l border-white/10 z-50 p-4 animate-slideInRight overflow-y-auto custom-scrollbar">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-3">
                            <label className="text-xs font-bold text-slate-500 uppercase flex justify-between">Brightness <span>{filters.brightness}%</span></label>
                            <input type="range" min="50" max="150" value={filters.brightness} onChange={(e) => setFilters({...filters, brightness: Number(e.target.value)})} className="w-full accent-primary h-1.5 bg-white/10 rounded-lg cursor-pointer" />
                        </div>
                        <div className="space-y-3">
                            <label className="text-xs font-bold text-slate-500 uppercase flex justify-between">Contrast <span>{filters.contrast}%</span></label>
                            <input type="range" min="50" max="150" value={filters.contrast} onChange={(e) => setFilters({...filters, contrast: Number(e.target.value)})} className="w-full accent-primary h-1.5 bg-white/10 rounded-lg cursor-pointer" />
                        </div>
                        <div className="space-y-3">
                            <label className="text-xs font-bold text-slate-500 uppercase flex justify-between">Saturation <span>{filters.saturate}%</span></label>
                            <input type="range" min="0" max="200" value={filters.saturate} onChange={(e) => setFilters({...filters, saturate: Number(e.target.value)})} className="w-full accent-primary h-1.5 bg-white/10 rounded-lg cursor-pointer" />
                        </div>
                        <div className="space-y-3">
                            <label className="text-xs font-bold text-slate-500 uppercase flex justify-between">Zoom <span>{Math.round(zoom * 100)}%</span></label>
                            <div className="flex gap-2">
                                <button onClick={() => setZoom(Math.max(1, zoom - 0.1))} className="p-1 bg-white/5 rounded hover:bg-white/10"><ZoomOut className="w-4 h-4" /></button>
                                <input type="range" min="1" max="2" step="0.1" value={zoom} onChange={(e) => setZoom(Number(e.target.value))} className="flex-1 accent-primary h-1.5 bg-white/10 rounded-lg cursor-pointer" />
                                <button onClick={() => setZoom(Math.min(2, zoom + 0.1))} className="p-1 bg-white/5 rounded hover:bg-white/10"><ZoomIn className="w-4 h-4" /></button>
                            </div>
                        </div>

                        {/* Toggles */}
                        <div className="pt-4 border-t border-white/10 space-y-2">
                            <button onClick={() => setShowStats(!showStats)} className={`w-full flex items-center justify-between p-3 rounded-lg text-sm ${showStats ? 'bg-primary/20 text-primary' : 'bg-white/5 text-slate-300'}`}>
                                <span className="flex items-center gap-2"><Activity className="w-4 h-4" /> Stats for Nerds</span>
                                {showStats && <Check className="w-4 h-4" />}
                            </button>
                            <button onClick={toggleAudioBoost} className={`w-full flex items-center justify-between p-3 rounded-lg text-sm ${boostEnabled ? 'bg-red-500/20 text-red-400' : 'bg-white/5 text-slate-300'}`}>
                                <span className="flex items-center gap-2"><Zap className="w-4 h-4" /> Audio Boost</span>
                                {boostEnabled && <Check className="w-4 h-4" />}
                            </button>
                            <button onClick={takeSnapshot} className="w-full flex items-center justify-between p-3 rounded-lg text-sm bg-white/5 hover:bg-white/10 text-slate-300">
                                <span className="flex items-center gap-2"><Camera className="w-4 h-4" /> Take Snapshot</span>
                            </button>
                            <button onClick={resetFilters} className="w-full p-3 rounded-lg text-sm bg-white/5 text-slate-400 hover:text-white mt-2">
                                Reset All Effects
                            </button>
                        </div>
                    </div>
            ...
            <div className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-4 transition-opacity duration-300 z-40 ${showControls || !playing ? 'opacity-100' : 'opacity-0'}`}>
                ...
                <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2 group/volume relative">
                            ...
                            <div className="w-0 overflow-hidden group-hover/volume:w-24 transition-all duration-300 flex items-center">
                                <input 
                                    ...
                                    className="w-24 h-1 accent-white ml-2"
                                />
                            </div>
                        </div>
                    ...
                    <div className="flex items-center gap-3 text-white relative">
                        ...
                        {showSettingsMenu && !showProSettings && (
                            <div className="absolute bottom-12 right-0 bg-dark-900/95 backdrop-blur-md border border-white/10 rounded-xl p-2 min-w-[200px] animate-slideUp shadow-xl flex flex-col gap-2">
                                <div className="flex items-center justify-between px-2 pb-2 border-b border-white/10">
                                    <span className="text-xs font-bold text-slate-400 uppercase">Settings</span>
                                    <button onClick={() => setShowSettingsMenu(false)}><X className="w-3 h-3 text-slate-400"/></button>
                                </div>
                                
                                <div className="p-2">
                                    <div className="flex items-center justify-between text-sm mb-1">
                                        <span className="flex items-center gap-2"><Gauge className="w-3 h-3"/> Speed</span>
                                    ...
                                    <input 
                                        ...
                                        className="w-full accent-primary h-1 bg-white/20 rounded cursor-pointer" 
                                    />
                                </div>

                                <button onClick={() => { setShowProSettings(true); setShowSettingsMenu(false); }} className="flex items-center gap-2 px-2 py-2 hover:bg-white/5 rounded text-sm text-yellow-400 font-bold">
                                    <Sliders className="w-3 h-3" /> Pro Settings
                                </button>
                                ...
                                {qualities.length > 0 && (
                                    <div className="flex flex-col gap-1 mt-1 border-t border-white/10 pt-2">
                                        ...
                                        <button onClick={() => changeQuality(-1)} className={`text-left px-3 py-1.5 rounded text-sm flex justify-between ${currentQuality === -1 ? 'bg-primary/20 text-primary' : 'text-white hover:bg-white/5'}`}>
                                            Auto {currentQuality === -1 && <Check className="w-3 h-3"/>}
                                        </button>
                                        {qualities.map(q => (
                                            <button key={q.level} onClick={() => changeQuality(q.level)} className={`text-left px-3 py-1.5 rounded text-sm flex justify-between ${currentQuality === q.level ? 'bg-primary/20 text-primary' : 'text-white hover:bg-white/5'}`}>
                        ...
                        <button onClick={() => setLightsOff(!lightsOff)} className={`p-1 ${lightsOff ? 'text-yellow-400' : 'hover:text-primary'} transition-colors`} title="Lights Off">
                            {lightsOff ? <Lightbulb className="w-5 h-5 fill-current" /> : <LightbulbOff className="w-5 h-5" />}
                        </button>

                        <button onClick={togglePiP} className="hover:text-primary transition-colors hidden sm:block p-1" title="Picture in Picture">
                            <PictureInPicture className="w-5 h-5" />
                        </button>

                        <button onClick={() => setShowSettingsMenu(!showSettingsMenu)} className={`hover:text-primary transition-colors p-1 ${showSettingsMenu ? 'text-primary' : ''}`} title="Settings">
